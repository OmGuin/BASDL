...
#directory
data_dir = os.path.join("data_gen_phase1", "pchdata")
...
def SimPhotDiffFlowGL6(C_molar, Rp, D, totalTime, binDt, w0, axialFactor, includeBg, bgRate, beamShape, vFlow, rng, resFactor=10):
    ...
    return arrivalTimes, counts, timeBins, Rp_i_out, sigma_b
...
def run_one_sim(sim_num):
    ...
    #run sims
    arrivalTimes1, _, _, Rp_i1, sigma_b_1 = SimPhotDiffFlowGL6(C_molar = conc1,
                                                    Rp = AmpS1 / setDt, 
                                                    D = D, 
                                                    totalTime = tt, 
                                                    binDt = dt, 
    ...
    sigma_b1 = sigma_b_1
    if num_species >= 2:
        arrivalTimes2, _, _, Rp_i2, sigma_b_2 = SimPhotDiffFlowGL6(C_molar = conc2,
                                                        Rp = AmpS2 / setDt, 
                                                        D = D, 
                                                        totalTime = tt, 
                                                        binDt = dt, 
        ...
        sigma_b2 = sigma_b_2
    if num_species == 3:
        arrivalTimes3, _, _, Rp_i3, sigma_b_3 = SimPhotDiffFlowGL6(C_molar = conc3,
                                                        Rp = AmpS3 / setDt, 
                                                        D = D, 
                                                        totalTime = tt, 
                                                        binDt = dt, 
    ...
    PCHedges = np.logspace(np.log10(3), np.log10(6000), 11)
